...
- 2026-09-08: Mobile.html (學員報到介面) 的統計提示最終定為「已報到: N 人, 學員總人數: T 人」— 參與率與未報到兩項都移除 (`updateStats` 內 `rate`/`unchecked` 變數連同 `__prevStatsMobile` 對應欄位一併清掉, bump 動畫改看 checked/total 是否變動). Desktop.html 管理端報到頁的今日統計 (選課後的初次顯示 + `updateTodayStats` 兩處) 同步改成一樣的字串, 兩端一致. 分三次交付: 先拿掉參與率 (ff0fc85) → 未報到改成學員總人數 (86d49b4) → 補上 Desktop 端. **教訓: 這類顯示字串的調整, 她要的是 Desktop/Mobile 兩端一起改, 不要因為她只提「學員報到介面」就只動一邊.** 備份 `Mobile.html.bak-20260908-160148` / `Mobile.html.bak2-20260908-162645` / `Desktop.html.bak5-20260908-170736`.
- 2026-09-08: **UI 術語 Team → Dept** (Astrid 定案: Dept = Team, DB 欄位仍是 `team`, 未改 schema). 兩端搜尋框 placeholder 統一為「搜尋學員(Function/Dept/Name/ID)」(Mobile `#q` / Desktop 報到頁 `#studentSearch` / 學員名單 `#stuQuery`), 學員名單表頭 Team→Dept, 另含補登頁搜尋框與表頭、自助建立學員表單 label/placeholder、新增/編輯學員 modal label、三則提示訊息. **CSV 與 xlsx 的欄位名仍是 `Team` 不動** — 匯入端讀的是使用者檔案的 `r['Team']||r['team']`, 改了既有 CSV 就匯不進來; 要改必須匯出入同步改並讓匯入相容舊表頭. 內部變數 (`inpTeam`/`sortedTeams`/`tdTeam`) 未動. 備份 `Desktop.html.bak6-20260908-171143` / `Mobile.html.bak3-20260908-171143`.
- 2026-09-08: **課程的「上課學員單位 Dept」改名為 Function, 並改成可複選 chips** (Astrid 定案, 起因是學員的 Team→Dept 與課程既有的 `courses.dept` 撞名). 選項來源 = 學員名單既有的 Function 值 (Desktop 用 `auth:true` 撈 students, index 沿用 `loadStudentsCache` 匿名快取); 多選值以**逗號分隔字串**寫回 `courses.dept` 既有欄位, **未改 schema**. 實作: 兩份 `showFormModal` 新增 `type:'multiselect'` (畫 chips + hidden input 帶 data-key, 送出邏輯不變), 共用 `renderFunctionChips`/`fetchFunctionOptions`/`parseCsvVals`; 涵蓋 Desktop 建立課程表單 + 編輯課程 modal, index 月曆的 Add + 編輯課程. 顯示端: 多值以 ` · ` 分隔 (`fmtDeptText`), 月曆 chip 顏色改用第一個 Function (`firstDeptVal`) 以維持同單位同色. 課程原有值若不在學員 Function 清單裡會一併列出, 不會被編輯清掉. **匯出入的欄位名一律維持原樣, 已定案不改** (2026-09-08 Astrid: 「xlsx匯出匯入不用改名, 只有我是管理員, 我清楚就可以了」) — xlsx 的「部門」欄 (課程的 Function) 與「Team」欄 (學員的 Dept)、CSV 匯出入的 `Team` 表頭與匯入下拉的說明文字, 全部保持現狀, **不要再提議改名**. 已知風險 (可接受): 若有人照 UI 的 Dept 自製 CSV 表頭, 匯入端 `r['Team']||r['team']` 讀不到會使 team 變空, 覆蓋模式會清空既有值 — 因為只有管理員自己匯入, 風險由她自行掌握. 備份 `Desktop.html.bak7-20260908-174057` / `index.html.bak2-20260908-174057`.
...
- 2026-09-08 驗收回報 (Astrid): ✅ 兩處 Excel 匯出鈕正常 ✅ 2026-08-05 CQE Agent P2 start_time 已改回 14:00. 追加調整: 單位 chips 改成**一列 + 水平 scroll** (`.cqe-ms` 改 `flex-wrap:nowrap` + `overflow-x:auto`, 附深色 scrollbar; 專案第一處自訂 scrollbar, G4 的 R7 全站深色化仍待做). 另修掉 `fetchFunctionOptions` 讀取失敗時把空陣列寫進快取的缺陷 — 一次失敗後 chips 會永遠空白且不再重試, 現在失敗只回 [] 並 console.error.
- 待 Astrid: ①首次真實使用「取消報到」時挑一筆測試再批次用 ②確認課程頁/月曆 Add 的 Function chips 有正確列出學員名單的 Function (本機連不到 Supabase, 無法實連驗證).
