...
- 2026-09-08: Mobile.html (學員報到介面) 的統計提示最終定為「已報到: N 人, 學員總人數: T 人」— 參與率與未報到兩項都移除 (`updateStats` 內 `rate`/`unchecked` 變數連同 `__prevStatsMobile` 對應欄位一併清掉, bump 動畫改看 checked/total 是否變動). 分兩次交付: 先拿掉參與率 (ff0fc85), 她再要求未報到改成學員總人數. Desktop.html 管理端的參與率 (第 1321/1491 行兩處) **未動** — 她只點名學員端. 備份 `Mobile.html.bak-20260908-160148` / `Mobile.html.bak2-20260908-162645`.
